...
export const Sidebar: React.FC = () => {
  ...
  const navItems = [
    ...
    { label: 'News Articles', href: '/dashboard/articles', icon: '📰' },
    ...
    <aside className="w-64 glass-panel h-screen flex flex-col justify-between p-4 sticky top-0 z-30 border-r border-slate-800">
      <div>
        <div className="flex items-center gap-3 px-3 py-4 mb-6 border-b border-slate-800/80">
          <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-sky-500 to-indigo-600 flex items-center justify-center font-bold text-white shadow-lg shadow-sky-500/30">
            AI
          </div>
          <div>
            <h1 className="font-bold text-slate-100 text-sm tracking-wide glow-blue">AI NEWS STUDIO</h1>
            <span className="text-[10px] text-sky-400 bg-sky-500/10 px-1.5 py-0.5 rounded border border-sky-500/20">AUTONOMOUS</span>
          </div>
        </div>
